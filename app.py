...
def get_task_audio_notes(task_id):
    try:
        logger.info(f"Fetching audio notes for task: {task_id}")
        current_user = get_jwt_identity()
        logger.info(f"Request by user: {current_user}")
        ...
        logger.info(f"Found {len(notes)} audio notes for task {task_id}")
        
        if not notes:
            logger.warning(f"No audio notes found for task {task_id}")
        ...
        logger.info(f"Returning {len(formatted_notes)} formatted audio notes")
        # Log the first note for debugging
        if formatted_notes:
            logger.debug(f"First note: audio_id={formatted_notes[0].get('audio_id')}, file_name={formatted_notes[0].get('file_name')}")
